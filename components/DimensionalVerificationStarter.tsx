...
export default function DimensionalVerificationStarter({
  ...
  async function createVerification(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const scaleNames = SUBTYPE_SCALES[subtype];
      const scaleRows = scaleNames.map((scaleName, index) => ({
        calibration_record_id: insertedRecord.id,
        scale_order: index + 1,
      ...
      router.push(`/verifiche/${insertedRecord.id}/misure-dimensionale`);
      router.refresh();
    } catch (error) {
      const message =
    ...
    <form onSubmit={createVerification} className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">
          Dati iniziali verifica dimensionale
        </h2>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <label className="space-y-1 xl:col-span-3">
            ...
            <select
              value={subtype}
              onChange={(event) =>
                setSubtype(event.target.value as DimensionalSubtype)
              }
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {isSaving ? "Creazione..." : "Crea verifica dimensionale"}
        </button>
      </div>
    </form>
  );
