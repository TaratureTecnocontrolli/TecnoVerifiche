...
export default function TorqueVerificationStarter({
  ...
  const torqueReferenceInstruments = useMemo(() => {
  ...
  const availableReferenceInstruments =
    torqueReferenceInstruments.length > 0
      ? torqueReferenceInstruments
      : referenceInstruments;
  ...
  async function createVerification(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!selectedReferenceInstrument) {
        throw new Error("Seleziona lo strumento campione da utilizzare.");
      }

      if (selectedReferenceBlocked) {
        throw new Error(
          "Lo strumento campione selezionato è scaduto o fuori servizio. Seleziona un campione valido."
        );
      }
      ...
      const referenceName = selectedReferenceInstrument.name || "Strumento campione";
      ...
      const { error: reportDetailsError } = await supabase
        ...
        .insert({
          ...
          requested_tests: buildTorqueRequestedTestsText(),
          premise_text: buildTorquePremiseText(),
      ...
      router.push(`/verifiche/${insertedRecord.id}/misure-dinamometria`);
      router.refresh();
    } catch (error) {
      const message =
  ...
  return (
    <form onSubmit={createVerification} className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {selectedReferenceBlocked && (
            <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
              Questo strumento campione non è utilizzabile perché risulta
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {isSaving ? "Creazione..." : "Crea verifica coppia"}
        </button>
      </div>
    </form>
  );
