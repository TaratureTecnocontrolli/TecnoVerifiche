...
import { getMassReportDefaults } from "@/lib/report-defaults";
...
const MASS_SCALES = [
  { order: 1, name: "Ripetibilità" },
  { order: 2, name: "Eccentricità" },
  { order: 3, name: "Linearità" },
];
...
export default function MassVerificationStarter({
  ...
  const massReferenceInstruments = useMemo(() => {
  ...
  const availableReferenceInstruments =
    massReferenceInstruments.length > 0
      ? massReferenceInstruments
      : referenceInstruments;
  ...
  async function createVerification(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!selectedReferenceInstrument) {
        throw new Error("Seleziona la massa campione da utilizzare.");
      }

      if (selectedReferenceBlocked) {
        throw new Error(
          "La massa campione selezionata è scaduta o fuori servizio. Seleziona un campione valido."
        );
      }
      ...
      const referenceName = selectedReferenceInstrument.name || "Massa campione";
      ...
      const scaleRows = MASS_SCALES.map((massScale) => ({
        calibration_record_id: insertedRecord.id,
        scale_order: massScale.order,
        scale_name: massScale.name,
        scale_range:
      ...
      router.push(`/verifiche/${insertedRecord.id}/misure-massa`);
      router.refresh();
    } catch (error) {
      const message =
  ...
  return (
    <form onSubmit={createVerification} className="space-y-6">
      ...
      {selectedReferenceInstrument && selectedReferenceStatus && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">
                Anteprima massa campione
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Questo snapshot verrà salvato nella verifica e riutilizzato
                sulle tre prove (ripetibilità, eccentricità, linearità).
              </p>
            </div>
          ...
          {selectedReferenceBlocked && (
            <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
              Questa massa campione non è utilizzabile perché risulta scaduta
              o fuori servizio.
            </div>
          )}
        </section>
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {isSaving ? "Creazione..." : "Crea verifica massa"}
        </button>
      </div>
    </form>
  );
