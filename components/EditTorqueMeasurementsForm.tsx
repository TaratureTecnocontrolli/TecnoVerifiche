...
type EditTorqueMeasurementsFormProps = {
  recordId: string;
...
function buildEditableScale(
  ...
  return {
    ...
    points: scaleMeasurements.map((measurement) => ({
      ...
      nominalTorque: numberToInputValue(measurement.nominal_value),
...
function calculateTorquePoint(point: EditableTorquePoint): CalculatedTorquePoint {
  ...
  const error = average - nominalTorque;
  const errorPercent =
    nominalTorque !== 0 ? (error / nominalTorque) * 100 : 0;
  const repeatabilityPercent =
    nominalTorque !== 0 ? ((max - min) / nominalTorque) * 100 : 0;
...
export default function EditTorqueMeasurementsForm({
  recordId,
  recordNumber,
  reportStatus,
  initialScales,
  ...
  isInternal = false,
}: EditTorqueMeasurementsFormProps) {
  ...
  function updatePoint(
    pointId: string,
    field: keyof Omit<EditableTorquePoint, "id" | "measurementId">,
    value: string
  ) {
  ...
  function addPoint() {
    ...
    setScale((currentScale) => {
      ...
      const nextNominalValue = lastPoint
        ? toNumber(lastPoint.nominalTorque) + 10
        : 10;
      ...
      return {
        ...
        points: [
          ...currentScale.points,
          {
            ...
            measurementId: null,
            nominalTorque: numberToInputValue(nextNominalValue),
            reading1: "",
            reading2: "",
            reading3: "",
            tolerancePercent: "",
    ...
    try {
      ...
      for (let pointIndex = 0; pointIndex < calculatedPoints.length; pointIndex += 1) {
        ...
        const payload = {
          ...
          nominal_value: calculatedPoint.nominalTorque,
          applied_value: calculatedPoint.average,
      ...
      setSaveMessage("Misure dinamometriche aggiornate correttamente.");
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Errore durante il salvataggio delle misure dinamometriche.";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href={isInternal ? "/verifiche/" + recordId + "/rapportino-interno" : "/verifiche/" + recordId + "/rapporto"}
              className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
            >
              {isInternal ? "Rapportino" : "Dati rapporto"}
            </Link>

            <Link
              href={isInternal ? "/verifiche/" + recordId + "/rapportino-interno" : "/verifiche/" + recordId + "/rapporto/finale"}
              className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700"
            >
              {isInternal ? "Rapportino" : "Rapporto"}
            </Link>
          </div>
        </div>
      </section>
      ...
      <fieldset disabled={isReadOnly} className="space-y-6 disabled:opacity-70">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="border-t border-slate-200 p-5">
            <SimpleAccuracyChart
              title="Grafico errore accuratezza % - Dinamometria"
              lineColor="#d97706"
              points={calculatedPoints.map((point, index) => ({
                label:
                  point.nominalTorque
                    ? formatItalianNumber(point.nominalTorque, 3)
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">
            Salva misure dinamometriche
          </h2>
          <p className="text-sm text-slate-500">
            I valori vengono salvati nella tabella misure usando coppia nominale,
