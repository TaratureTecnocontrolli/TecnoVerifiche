...
import { getFlowReportDefaults } from "@/lib/report-defaults";
...
export default function FlowVerificationStarter({
  ...
  const flowReferenceInstruments = useMemo(() => {
  ...
  const availableReferenceInstruments =
    flowReferenceInstruments.length > 0
      ? flowReferenceInstruments
      : referenceInstruments;
  ...
  async function createVerification(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/verifiche/${insertedRecord.id}/misure-portata`);
      router.refresh();
    } catch (error) {
      const message =
    ...
    <form onSubmit={createVerification} className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">
          Dati iniziali verifica portata
        </h2>
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {isSaving ? "Creazione..." : "Crea verifica portata"}
        </button>
      </div>
    </form>
  );
