...
import EditSclerometricMeasurementsForm from "@/components/EditSclerometricMeasurementsForm";
...
export default async function EditSclerometricMeasurementsPage({
  params,
}: PageProps) {
  ...
  const { data: recordData, error: recordError } = await supabase
    ...
    .select("id, record_number, mode, verification_module, report_status")
    .eq("id", id)
    .single();
  ...
  if (
    record.verification_module !== "SCLEROMETRIC" &&
    record.mode !== "sclerometro"
  ) {
    notFound();
  }
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-slate-950">
              Misure prova sclerometrica
            </h1>

            <p className="mt-2 max-w-3xl text-slate-600">
              Inserisci le battute sull&apos;incudine di riferimento, le tre
              letture ripetute e controlla errore ed errore percentuale.
            </p>
          </div>
        ...
        <EditSclerometricMeasurementsForm
          recordId={id}
